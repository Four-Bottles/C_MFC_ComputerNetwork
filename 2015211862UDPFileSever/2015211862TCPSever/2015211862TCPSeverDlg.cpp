...
DWORD WINAPI ListenThread(LPVOID lpparam) {
	...
	*/
	...
	//读取mp3文件
	FILE *fp = NULL;
	if (fopen_s(&fp, "小幸运.mp3", "rb") != 0)
	{
		AfxMessageBox(_T("Open Mp3 File Failed1!"));
		return 0;
	}
	...
	while (!feof(fp))
	{
		...
		if (recvfrom(m_Listening, szBuf, sizeof szBuf, 0, (struct sockaddr*)&ClientAddr, &nLen) != SOCKET_ERROR)
		{
			memset(dataBuf, 0, MAX_LENGTH);//初始化缓冲区
			int iBytesRead = fread(dataBuf, 1, 1024, fp);
			int iRet = sendto(m_Listening, dataBuf, iBytesRead, 0, (struct sockaddr*)&ClientAddr, sizeof ClientAddr);
			//, buff, sizeof buff, 0,
			//CString siRet;
			//siRet.Format(_T("%d"),iRet);
			//AfxMessageBox(siRet);
			if (iRet <= 0)
			{
				break;
			}
			pDlg->m_edtMessage.SetWindowTextW(char2CSt(dataBuf));
			char szBuf[MAX_PATH];//数据缓冲区	
			memset(szBuf, 0, MAX_PATH);//初始化缓冲区
	...
	fclose(fp);
	memset(dataBuf, 0, MAX_LENGTH);//清空缓冲区
	sendto(m_Listening, dataBuf, 0, 0, (struct sockaddr*)&ClientAddr, sizeof ClientAddr);
...
void CMy2015211862TCPSeverDlg::OnBnClickedButtonStart()
{
	...
	m_Listening = socket(AF_INET, SOCK_DGRAM, 0);//初始化Socket，采用TCP方式-SOCK_STREAM
