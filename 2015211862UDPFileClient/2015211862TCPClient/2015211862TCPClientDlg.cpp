...
void CMy2015211862TCPClientDlg::OnBnClickedButtonSend()
{
	...
	*/
	...
	while (1)
	{
		...
		//AfxMessageBox(_T("start Recv"));
		memset(dataBuff, 0, MAX_LENGTH);
		len = recvfrom(socketTmp, dataBuff, sizeof dataBuff, 0, (struct sockaddr*)&local, &nlen);
		if (len != SOCKET_ERROR)//读取服务器端返回的数据
		{
			//AfxMessageBox(_T("Recving!"));
			CString s = char2CSt(szText);
			if (len <= 0)
			{
